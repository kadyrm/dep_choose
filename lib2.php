...
$result = get_score_above($tblname, 130, "exact sciences");
...
function get_score_above($tbl, $score, $exam){
        $query = "SELECT * FROM `".$tbl."` WHERE `score_min` >= ".$score." AND `exam_type`"."='".$exam."'";      
         mysql_query("SET NAMES UTF8");
        //execute the SQL query and return records
...
function query_build($tbl_name){
    $score = 130;
    $query = query_build_greater($tbl_name, "score", $score);
